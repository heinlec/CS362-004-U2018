...
int main() {
	...
	printf("Player 1 deck count should be two fewer...\n");
	...
	for (i = 0; i < originalState.handCount[thisPlayer]; i++) {
                card = originalState.hand[thisPlayer][i];
                if (card == copper || card == silver || card == gold) {
                        origTreasure++;
                }
	}
	myAssertFunction(currTreasure, origTreasure+2);
