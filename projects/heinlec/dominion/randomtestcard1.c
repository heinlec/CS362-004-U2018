...
int main() {
	...
	state.hand[thisPlayer][0] = kingdom[1];
	...
	int handDiff = &state.handCount - &originalState.handCount;
	int deckDiff = &state.deckCount - &originalState.deckCount;
	if (handDiff == 2 && deckDiff == 3) {
		numOfPasses++;
	//	printf("Passed\n");
	}
	else {
